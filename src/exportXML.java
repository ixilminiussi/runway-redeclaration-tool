...
public class exportXML {
    ...
    public static void exportBothToXML(String filename, ArrayList<Obstruction> obstructions, ArrayList<Runway> runways) throws IllegalArgumentException, Exception {
        // check filename is .xml
        try {
            if (!filename.substring(filename.length() - 4).equals(".xml")) {
                throw new IllegalArgumentException("file extension not XML");
        ...
        try {
            ...
            doc.appendChild(both);

            // root element
            Element obstructionsElement = doc.createElement("obstructions");
            both.appendChild(obstructionsElement);

            for (int i = 0, len = obstructions.size(); i < len; i++) {
                // obstruction element
                Element obstruction = doc.createElement("obstruction");
                obstructionsElement.appendChild(obstruction);
            ...
            both.appendChild(runwaysElement);

            for (int i = 0, len = runways.size(); i < len; i++) {
                // runway element
                Element runway = doc.createElement("runway");
                runwaysElement.appendChild(runway);
